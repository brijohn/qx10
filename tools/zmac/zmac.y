...
void emitjr(int opcode, struct expr *dest)
{
	...
	// in those cases.
	// When PHASE is active, $ is absolute so skip segment checking
	if (((relopt && !phaseflag && (dest->e_scope & SCOPE_SEGMASK) != segment) ||
	...
	label.part ARGPSEUDO arg_on ARG arg_off '\n' {
		...
		case PSINC:	/* include file */
			if (mras) {
				...
				// M80: append .mac if no extension present
				if (!strchr(tempbuf, '.'))
					strcat(tempbuf, ".mac");
			}
		...
		case PSIMPORT:	/* import file */
			...
			break;
		case PSMACLIB:
			// M80: append .lib if no extension present
			if (!strchr(tempbuf, '.'))
				strcat(tempbuf, ".lib");
			next_source(tempbuf, 1);
			break;
...
|
	symbol maybecolon {
		...
		// Per M80: labels within PHASE are absolute
		$1->i_scope |= phaseflag ? 0 : segment;
		if ($2 == 2)
			$1->i_scope |= SCOPE_PUBLIC;
	...
	'$'
		{
			...
			// When PHASE is active, $ represents an absolute address
			// per M80 manual: "All labels within a .PHASE block are defined
			// as the absolute value from the origin of the phase area"
			$$->e_scope = phaseflag ? 0 : segment;
		}
|
	UNDECLARED
		{
