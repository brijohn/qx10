 ...
 * encoding. Header[0x52-0x53] are a magic number. */
	...
	diaghead(diagfunc, "DiskCopy 4.2 disk image");
	if (is_macbin)
	{
		diagrawrange(diagfunc, fp, 0x00, 0x80, "MacBinary header");
		fpos = 0x80;
	}
	else	fpos = 0;
	diaghex(diagfunc, fpos, header, 1, "Description length");
	diaghex(diagfunc, fpos +  1, header +  1, 63, "Description");
	diaghex(diagfunc, fpos + 64, header + 64,  4, "Data bytes"); 
	diaghex(diagfunc, fpos + 68, header + 68,  4, "Tag bytes"); 
	diaghex(diagfunc, fpos + 72, header + 72,  4, "Data checksum"); 
	diaghex(diagfunc, fpos + 76, header + 76,  4, "Tag checksum"); 
	diaghex(diagfunc, fpos + 80, header + 80,  1, "Encoding"); 
	diaghex(diagfunc, fpos + 81, header + 81,  1, "Sidedness / GCR type / MFM sector size"); 
	diaghex(diagfunc, fpos + 82, header + 82,  2, "Magic number"); 
...
/* On a GCR variable-speed disk, vary the number of sectors */
			...
			diaghead(diagfunc, "Cylinder %d head %d (%d sectors)",
				cylinder, head, secs);
...
/* Migrate sectors */
			for (sector = 0; sector < secs; sector++)
			{
				...
				if (trail)
				{
					...
					diagnl(diagfunc);
					diaghex(diagfunc, tagpos,
				  	     secbuf + secsize, 12, 
					     "Sector %d tags", sector);
					tagpos = ftell(fp);
				}
				/* Load sector body */
				...
				diaghex(diagfunc, datapos, secbuf, secsize, 
		...
		if (!err)
		{
			...
			if (rsrclen)
			{
				...
				if (rsrcbuf)
				{
					...
					diagnl(diagfunc);
					diaghex(diagfunc, rsrcaddr, rsrcbuf,
						rsrclen, "Resource fork");
					dsk_free(rsrcbuf);
				}
			}
		}
