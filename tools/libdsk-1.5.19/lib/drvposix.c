 ...
 * and under UNIX, the entire directory is filled with zeroes. */
	...
	diaghead(diagfunc, "Raw / POSIX disk image");
	diagraw(diagfunc, pxself->px_fp, "Raw data");
	return DSK_ERR_OK;
}
...
dsk_err_t posix_openalt(DSK_DRIVER *self, const char *filename,
	DSK_REPORTFUNC diagfunc)
{
	return posix_open(self, filename, SIDES_ALT, diagfunc);
}

dsk_err_t posix_openoo(DSK_DRIVER *self, const char *filename,
	DSK_REPORTFUNC diagfunc)
{
	return posix_open(self, filename, SIDES_OUTOUT, diagfunc);
}

dsk_err_t posix_openob(DSK_DRIVER *self, const char *filename,
	DSK_REPORTFUNC diagfunc)
{
	return posix_open(self, filename, SIDES_OUTBACK, diagfunc);
}
