			...
			DSK_REPORTFUNC diagfunc)
{
	...
	if (diagfunc)
	{
		...
		diaghead(diagfunc, "JV3 disk image");
		diaghead(diagfunc, "Header area");
		for (n = 0; n < JV3_HEADER_LEN - 1; n += 3)
		{
			diaghex(diagfunc, n, self->jv3_header + n, 1, 
				"Cylinder %d", self->jv3_header[n]);
			diaghex(diagfunc, n + 1, self->jv3_header + n + 1, 1, 
				"Sector %d", self->jv3_header[n + 1]);
			diaghex(diagfunc, n + 2, self->jv3_header + n + 2, 1, 
				"Flags");
		}
		diaghex(diagfunc, JV3_HEADER_LEN - 1, self->jv3_header + 
				JV3_HEADER_LEN - 1, 1, "Read-only flag");
	}
	...
	do
	{
		for (n = 0; n < JV3_HEADER_COUNT; n++)
		{
			...
			diaghex(diagfunc, fpos, secbuf, secsize,
				"Data for cylinder %d sector %d", cyl, sec);
		...
		if (diagfunc)
		{
			...
			diaghead(diagfunc, "Additional header");
			for (n = 0; n < JV3_HEADER_LEN - 1; n += 3)
			{
				diaghex(diagfunc, fpos + n, 
					self->jv3_header + n, 1, 
					"Cylinder %d", self->jv3_header[n]);
				diaghex(diagfunc, fpos + n + 1, 
					self->jv3_header + n + 1, 1, 
					"Sector %d", self->jv3_header[n + 1]);
				diaghex(diagfunc, fpos + n + 2, 
					self->jv3_header + n + 2, 1, "Flags");
			}
			diaghex(diagfunc, fpos + JV3_HEADER_LEN - 1, 
				self->jv3_header + JV3_HEADER_LEN - 1, 1, 
				"Padding");
			fpos += JV3_HEADER_LEN;
		}
	}
