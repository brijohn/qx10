...
static IReporter *gl_diagrep = NULL;

void diag_on(const char *s)
{
	if (!gl_diagrep) return;
	CComBSTR bs(s);
	gl_diagrep->report(bs);
}
...
{
	if (!gl_diagrep) return;
	gl_diagrep->endreport();
}

STDMETHODIMP CLibrary::diag_open(BSTR f, IDispatch *reporter, BSTR d, BSTR c, IDisk **ppDisk)
{
	DSK_PDRIVER pd;
	dsk_err_t err;
	char *filename;
	...
	// Check that the reporter callback is actually an IReporter.
	IReporter *pReporter;
	HRESULT hr;

	hr = reporter->QueryInterface(IID_IReporter, (void **)&pReporter);
	...
	gl_diagrep = pReporter;
	err = dsk_diagopen(&pd, filename, driver, compress, diag_on, diag_off);
	gl_diagrep = NULL;
	pReporter->Release();
	...
	hr = CDisk::CreateInstance(ppDisk);
	if (FAILED(hr)) return hr;
